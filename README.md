...
- 支持填写图床图片 URL 添加外链照片，访客浏览时直接从图床加载，减少服务器流量
...
- 亮色 / 暗色 / 自动主题模式、首页主图、访客评论开关与审核、上传开关、注册开关、水印开关配置
...
photo-memory/
  ...
  frontend/
    ...
    src/
      ...
      utils/
      assets/styles/
        variables.css        # 亮色 / 暗色主题底层色板
        theme-contract.css   # 统一主题变量接口
        main.css             # 全局基础样式和 Element Plus 主题覆盖
  README.md
```
...
也可以在上传页面选择“图片 URL”，填写已经上传到图床的图片地址。外链照片只会把 `originalUrl`、`mediumUrl`、`thumbnailUrl` 保存到数据库，不会下载到服务器，也不会自动生成缩略图或读取 EXIF；拍摄日期、尺寸、地点等信息可在上传时填写或后续编辑。
...
- 上传目录只保存图片文件，不执行脚本
- 外链 URL 模式要求使用 `http://` 或 `https://` 图片地址；图片是否支持防盗链、跨域显示和长期访问由图床服务决定
...
前端已整理为统一主题接口，方便后续整体调整颜色或新增主题。

核心文件：

- `frontend/src/assets/styles/variables.css`：定义每套主题的底层色板，目前包含 `html.light` 和 `html.dark`。
- `frontend/src/assets/styles/theme-contract.css`：主题接口层，将底层色板映射为组件使用的 `--theme-*` 变量。
- `frontend/src/assets/styles/main.css`：全局基础样式、Element Plus 控件主题覆盖和通用页面样式。
- `frontend/src/stores/settings.store.js`：读取后台主题设置，并应用 `light`、`dark` 或 `auto` 到 `document.documentElement`。
...
- `dark`：暗色主题
- `auto`：跟随系统外观
- 自定义主题：可在后台填写核心组件颜色，并保存多套主题方案；默认提供“简单模式”，把相近页面场景合并控制，高级模式才展开全部细项。
- 主题导入导出：后台可将当前主题方案导出为 JSON，也可导入 JSON 主题方案并保存到方案列表。
- 自定义主题启用后会覆盖全站主题接口，包括页面、后台、表格、按钮、标签、瀑布流、Hero、地图、灯箱、加载骨架、遮罩和错误页；此时亮色 / 暗色 / 自动仅作为关闭自定义主题后的备用预设。
- 对比度检测：后台会检查正文、按钮、标签、地图弹窗、悬浮导航、照片遮罩等常用组合的对比度，低对比度时给出可一键采用的建议色。
- 自定义主题会保存到 `SystemSetting` 的 `themeCustomColors` / `savedThemes` 中；前台读取公共设置后通过 CSS 变量即时应用。

组件开发规则：

- 组件、页面、弹层、地图、照片卡片、标签、按钮和表单控件应优先使用 `--theme-*` 变量。
- 不建议在组件内直接使用 `--primary`、`--surface`、`--text`、`--line` 等底层色板变量。
- 不建议在组件样式中写死 `#fff`、`#000`、`rgba(...)` 等颜色，除非该颜色属于照片遮罩、灯箱背景等主题接口中已经明确约定的特殊场景。
- 如果要新增主题色，优先在 `variables.css` 中增加新的主题选择器，再通过 `theme-contract.css` 暴露统一接口。
- 如果要新增一类组件颜色，例如新的图表、播放器或编辑器工具栏，应先在 `theme-contract.css` 中定义对应接口，再在组件中引用。

常用主题接口包括：

- `--theme-button-*`：按钮、单选按钮、分段控件
- `--theme-input-*`：输入框、选择器、文本域
- `--theme-switch-*`：开关
- `--theme-table-*`：表格
- `--theme-tag-*`：标签与标签云
- `--theme-map-*`：地图底图、缩放控件、弹窗和 marker
- `--theme-image-*`：照片卡片遮罩、悬停、EXIF、加载骨架
- `--theme-lightbox-*`：大图预览灯箱
- `--theme-dock-*`：右下角圆形导航
- `--theme-wall-*`：首页和照片墙瀑布流卡片
- `--theme-hero-*`：首页主图区域
...
4. 地图不显示：后台“系统设置 -> 地图底图”可切换高德地图、OpenStreetMap 或自定义 HTTPS 瓦片源；国内网络建议使用默认高德地图。

...
- 照片瀑布流、分页筛选、灯箱预览、懒加载
- 照片详情页：大图、参数、作者、相册、分类、标签、地图、评论、点赞、收藏、上一张/下一张
...
- 访客无需登录即可点赞或评论；点赞保存设备/IP 审计记录，评论邮箱仅后台可见
...
访客点赞请求会自动携带浏览器持久设备标识，服务端结合 IP 限制同一照片重复点赞。访客评论需提交 `guestName` 和 `guestEmail`，邮箱不会出现在公开评论响应中。
...
- 反向代理只有一层时设置 `TRUST_PROXY_HOPS=1`，以便点赞审计记录真实访客 IP；也可在后台“系统设置 -> 部署与代理”中调整 `TRUST_PROXY_HOPS`
